...
class ItemListView(LoginRequiredMixin, ListView):
    template_name = "bookmarks/view_card.html"
    model = Item
...
def list(request):
    ...
    context.update(get_common_data())
    return render(request, "bookmarks/view_list.html", context)
...
def edit_view(request, pk):
    ...
    if request.method == "POST":
        form = BookmarksForm(request.POST, instance=obj)
        if form.is_valid():
            bookmarks = form.save()
            ...
            if new_tags:
                ...
                for tag_name in tags:
                    ...
                    bookmarks.tags.add(tag)
            bookmarks.save()
            return redirect("bookmarks:index")
    else:
        form = BookmarksForm(instance=obj)
...
def quick_add_bookmark(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            bookmarks = form.save(commit=False)

            # OGP情報を設定
            bookmarks.og_title = form.cleaned_data.get("og_title", "")
            bookmarks.og_description = form.cleaned_data.get("og_description", "")
            bookmarks.og_type = form.cleaned_data.get("og_type", "")
            bookmarks.og_site_name = form.cleaned_data.get("og_site_name", "")
            bookmarks.favicon_url = form.cleaned_data.get("favicon_url", "")
            ...
            if og_image_url:
                try:
                    ...
                    if not og_image_url.startswith(("http://", "https://")):
                        og_image_url = urljoin(bookmarks.url, og_image_url)
        ...
        form = BookmarksForm(initial=initial_data)

    context = {"form": form}
    context.update(get_common_data())
    return render(request, "bookmarks/quick_add_bookmark.html", context)
